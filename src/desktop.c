...
static void desktop_swap_buffers(void)
{
    // ВАЖНО: раньше тут копировалось scr_width*scr_height пикселей ПОДРЯД,
    // как будто в каждой строке видеопамяти ровно scr_width*4 байт. Это
    // верно почти всегда в QEMU, но на реальном железе pitch (шаг строки)
    // часто больше — видеокарта выравнивает строки по границе памяти.
    // Из-за этого на настоящем железе картинка "съезжала по диагонали"
    // с каждой строкой. Теперь копируем построчно, используя реальный
    // scr_pitch как шаг в байтах между строками.
    ...
    for (uint32_t row = 0; row < copy_height; row++) {
        uint8_t* dst_row = frontbuffer + (uint64_t)row * scr_pitch;
        uint32_t* src_row = &backbuffer[row * scr_width];

        if (bytes_per_pixel == 4) {
            uint32_t* dst32 = (uint32_t*)dst_row;
            for (uint32_t col = 0; col < copy_width; col++) {
                dst32[col] = src_row[col];
            }
        } else if (bytes_per_pixel == 3) {
            // Редкий случай 24bpp-фреймбуфера — пишем по 3 байта на пиксель.
            for (uint32_t col = 0; col < copy_width; col++) {
                uint32_t px = src_row[col];
                dst_row[col * 3 + 0] = (uint8_t)(px & 0xFF);
                dst_row[col * 3 + 1] = (uint8_t)((px >> 8) & 0xFF);
                dst_row[col * 3 + 2] = (uint8_t)((px >> 16) & 0xFF);
            }
        }
    }
}
...
void render_layer_dock(int single_click)
{
    ...
    /*
     * Glass blur — чуть сильнее радиус, чтобы "матовость" читалась заметнее
     */
    ...
    /*
     * Glass background — более прозрачный, чем раньше (macOS-style)
     */
    ...
    /*
     * Border — тоже чуть прозрачнее, чтобы не спорил с новой лёгкостью фона
     */
